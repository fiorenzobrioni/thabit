...
- Heatmap intensity = **fraction of the due suite that passed** that day, relative buckets like tsteps (quartiles of the user's own non-zero days); skipped tests leave the denominator; the future is blank and today is live; a day inside the suite's life with nothing to colour is a dim `·`.
...
| heatmap | stats | 12 weeks, intensity = due-suite pass fraction, relative buckets; `no run` days a dim `·`, the future and days before the suite blank |
