...
- [ ] `CodeBlockContainer`, `CodeCanvas` (gutter, indent 20px, `EditorOptions`), `EditorTab` + `EditorNavBar`, `TerminalStatusBar`, `TerminalInput`, `GlowFab` (glifo di default **`+`** — l'unico verbo col glow di thabit è aggiungere un test alla suite)
- [ ] `JsonSyntax` + `MarkdownSyntax` importati (settings.config e stats.md/README li usano)
- [ ] **`YamlSyntax` nuovo**: tokenizer per il file principale — chiavi, stringhe, liste `- `, checkbox `[x]`/`[ ]`/`[~]`/`[!]` come token colorati (verde/neutro/grigio/rosso), commenti `#`. È il primo tokenizer nuovo della serie: test dedicati sul modello di `JsonSyntaxTest`. **Decisione da VISION §1.1**: il canale commenti veste la sintassi del file ospite (`#` in YAML, `//` nel settings) — il tokenizer la rispetta
- [ ] `MarkdownTable` e `UnitFormat`-equivalente (qui serve solo formattazione numeri/percentuali/orari: valutare cosa importare davvero — non copiare ciò che non serve)
- [ ] Test Compose importati e adattati; nav bar con 4 destinazioni `editor`/`log`/`stats`/`settings` e glifi coerenti (per l'editor: il glifo checklist/`checklist` o `data_object`, decidere al momento con la label giusta)
- [ ] Shell `ThabitApp` provvisoria: bottom bar + placeholder per tab (`# <file> — not yet written`), senza Navigation Compose (il NavHost per-tab è della Fase 4)
