...
/**
 * The typing animation asserted instead of watched (Fase 19).
 *
...
class TypistTest {
    ...
    private val typist = Typist(
        listOf(
            TypedLine(line("$ thabit init"), msPerChar = 10, pauseAfterMs = 100),
    ...
    fun `the beat after a line holds the cursor where the line ended`() {
        ...
        assertEquals(1, shown.size)
        assertEquals("$ thabit init$BlockCursor", (shown.single() as CodeLine).text.text)
    }
